...
Tickspot.prototype._ensureDateString = function (date) {
    return _.isDate(date) ? date.getFullYear() + '-' + this._pad(date.getMonth()+1, 2) + '-' + this._pad(date.getDate(), 2) : date;
};

Tickspot.prototype._pad = function (number, size) {
    var output = '' + number;
...
Tickspot.prototype.entries = function (start_date, end_date, options, callback) {
    ...
    if (!_.isUndefined(end_date)) {
        if (_.isFunction(end_date)) {
            callback = end_date;
            end_date = void 0;
        } else if (_.isUndefined(options) && !_.isString(end_date) && !_.isDate(end_date)) {
            options = end_date;
            end_date = void 0;
        }
    }
